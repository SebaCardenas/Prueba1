...
def daño(PA, PD, A, D):
    ...
    if A.tipo in T1p.lista_categoria_physic and D.tipo in T1p.lista_categoria_physic:
        ...
        dano = ((2 * PA.nivel + 10) / 250) * (((PA.ataque_especial / PD.defensa_especial) * PA.ataque_especialbase) + 2) \
               * modificador
...
def actualizar(pokemon):
    ...
    for no in range(0, len(T1p.Jugador_actual.lista_programones)):
        if T1p.Jugador_actual.lista_programones[no]. \
                id == T1p.Programon_actual_jugador.id:
    ...
    for NombreProgramon in T1p.lista_de_todos_programones:
        if NombreProgramon.nombre == "Charmander":
            ...
            retorno = CpR.Programon(NombreProgramon.id, NombreProgramon.nombre, NombreProgramon.movimientos,
                                    NombreProgramon.tipo, NombreProgramon.vida,
                                    NombreProgramon.velocidad, NombreProgramon.ataque, NombreProgramon.ataque_especial,
                                    NombreProgramon.defensa,
                                    NombreProgramon.defensa_especial,
                                    NombreProgramon.id_evolucion,
...
def crear_salvaje():
    ...
    retorno = CpR.Programon(NombreProgramon.id, NombreProgramon.nombre, NombreProgramon.movimientos,
                            NombreProgramon.tipo, NombreProgramon.vida,
                            NombreProgramon.velocidad, NombreProgramon.ataque,
                            NombreProgramon.ataque_especial, NombreProgramon.defensa,
                            NombreProgramon.defensa_especial,
                            NombreProgramon.id_evolucion,
                            NombreProgramon.nivel_evolucion)
...
def comprobar_evol():
    if T1p.Programon_actual_jugador.nivel == T1p.Programon_actual_jugador.nivel_evolucion \
            and T1p.Programon_actual_jugador.nivel_evolucion != None:
        ...
        for evol in T1p.lista_de_todos_programones:
            if evol.id == T1p.Programon_actual_jugador.id_evolucion:
                for nos in range(0, len(T1p.Jugador_actual.lista_programones)):
                    if T1p.Jugador_actual.lista_programones[nos]. \
                            id == T1p.Programon_actual_jugador.id:
...
def registro(Nombre_Programon, Nombre_contrincante_programon, Dueño, resultado):
    ...
    #Register = namedtuple("Registro", ["NombreProgramon", "NombreProgramonContrincante", "Dueño", "Resultado"])
